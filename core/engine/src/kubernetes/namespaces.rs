...
pub fn map_namespace(ns: &Namespace) -> models::NamespaceInfo {
    ...
    if let Some(map) = ns.metadata.labels.clone() {
        labels = map;
    }

    let mut annotations = std::collections::BTreeMap::new();
    if let Some(map) = ns.metadata.annotations.clone() {
        annotations = map;
    }
