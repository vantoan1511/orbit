...
interface CreateSecretDialogData {
  initialNamespace?: string
}

const dialogRef = inject<
  | Ref<{
      data?: CreateSecretDialogData
      close: () => void
    }>
  | undefined
>('dialogRef')
...
const nameErrorMessage = computed(() => {
  ...
  const exists = k8sStore.secrets.some(
    (sec) => sec.namespace === namespace.value && sec.name.toLowerCase() === trimmed.toLowerCase()
  )
  if (exists) {
    return `A Secret named "${trimmed}" already exists in namespace "${namespace.value}".`
  }
  return null
})
...
<template>
  <form @submit.prevent="handleCreate" class="flex flex-col gap-3.5">
    <p class="text-xs text-muted-color">
      Create a new Kubernetes Secret to store sensitive key-value configuration data:
    </p>

    <!-- Name & Namespace -->
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
      <div class="flex flex-col gap-1.5">
        <label for="create-secret-name" class="text-xs font-semibold text-muted-color">
      ...
      <div class="flex flex-col gap-1.5">
        <label for="create-secret-namespace" class="text-xs font-semibold text-muted-color">
          Namespace <span class="text-(--danger)">*</span>
        </label>
        <Select
          id="create-secret-namespace"
          v-model="namespace"
    ...
    <!-- Secret Type -->
    <div class="flex flex-col gap-1.5">
      <label for="create-secret-type" class="text-xs font-semibold text-muted-color">
        Secret Type
      </label>
      <Select
