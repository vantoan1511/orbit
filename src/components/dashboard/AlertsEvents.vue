...
<template>
  <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
    <!-- Recent Warnings -->
    <div
      class="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-5 shadow-sm transition-all duration-200 flex flex-col justify-between"
    >
      <div>
        <div class="flex items-center justify-between mb-4">
          ...
          <button
            class="text-xs text-[var(--accent)] hover:underline flex items-center gap-1 font-medium"
          >
            <span>View all</span>
          </button>
        </div>
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between gap-2">
                <span class="text-xs font-bold text-[var(--text-primary)] font-mono">{{
                  warning.reason
                }}</span>
        ...
        <div class="divide-y divide-[var(--border)]">
          ...
          >
            ...
            <div class="flex-1 min-w-0 flex items-center justify-between gap-4">
              <p class="text-xs text-[var(--text-secondary)] truncate font-medium">
                {{ event.message }}
              </p>
              <span class="text-[10px] text-[var(--text-muted)] font-mono whitespace-nowrap">{{
                event.time
