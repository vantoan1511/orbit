...
// Data for Pod Health
const podHealth = {
  running: { count: 264, pct: 84, color: "bg-emerald-500", text: "text-emerald-500", dot: "bg-emerald-500" },
  pending: { count: 18, pct: 6, color: "bg-amber-500", text: "text-amber-500", dot: "bg-amber-500" },
...
const nodeHealth = {
  ready: { count: 12, pct: 100, color: "bg-emerald-500", text: "text-emerald-500", dot: "bg-emerald-500" },
...
<template>
  <div class="flex flex-col gap-6">
    <!-- Pod Health Card -->
    <div class="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-5 shadow-sm transition-all duration-200 flex flex-col justify-between h-[13.5rem]">
      <div>
      ...
      <div class="w-full h-3 rounded-full bg-[var(--bg-hover)] overflow-hidden flex">
        <div :style="{ width: podHealth.running.pct + '%' }" :class="podHealth.running.color" title="Running"></div>
        <div :style="{ width: podHealth.pending.pct + '%' }" :class="podHealth.pending.color" title="Pending"></div>
        <div :style="{ width: podHealth.failed.pct + '%' }" :class="podHealth.failed.color" title="Failed"></div>
        <div :style="{ width: podHealth.crashLoop.pct + '%' }" :class="podHealth.crashLoop.color" title="CrashLoop"></div>
      </div>
    </div>
