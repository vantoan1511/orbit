...
<template>
  <div
    class="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-5 shadow-sm transition-all duration-200 flex flex-col justify-between h-full"
  >
    <div>
      <!-- Header with Tabs -->
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-xs border-collapse">
          ...
          <tbody class="divide-y divide-[var(--border)]/50">
            ...
            >
              <td class="py-2.5 font-medium truncate max-w-[100px]" :title="item.pod">
                {{ item.pod }}
              </td>
              <td class="py-2.5 text-[var(--text-muted)]">{{ item.namespace }}</td>
              <td class="py-2.5 font-mono" :class="activeTab === 'restarts' ? 'text-right' : ''">
                {{ activeTab === 'cpu' ? item.value : activeTab === 'memory' ? '2.4 GiB' : '2' }}
              </td>
              <td v-if="activeTab !== 'restarts'" class="py-2.5 text-right font-mono">
                <div class="flex items-center justify-end gap-2">
                  <span class="w-8 text-right"
                    >{{ activeTab === 'cpu' ? item.pct : item.pct - 2 }}%</span
                  >
                  <div
                    class="w-16 h-1.5 rounded-full bg-[var(--bg-hover)] overflow-hidden hidden sm:block"
                  >
                    <div
                      :style="{ width: (activeTab === 'cpu' ? item.pct : item.pct - 2) * 2 + '%' }"
                      :class="item.color"
                      class="h-full"
                    ></div>
                  </div>
    ...
    <!-- View All Link -->
    <button
      class="text-xs text-[var(--accent)] hover:underline flex items-center justify-between font-semibold mt-4 border-t border-[var(--border)] pt-4 w-full"
    >
