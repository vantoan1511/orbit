...
const MenuItemButton = (props: { item: MenuItem; menuProps: any }) => {
  return h(
    'button',
    {
      ...
      class: [
        'flex items-center gap-2 px-3 py-2 w-full text-left text-xs text-(--text-secondary) hover:text-primary hover:bg-(--bg-hover)/20 rounded-md transition-colors cursor-pointer select-none',
        props.item.class
      ]
    },
    [
