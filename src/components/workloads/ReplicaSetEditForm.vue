...
import {
  ...
  type ReplicaSetContainerFormState
} from '@/utils/replicaSet'
...
const props = defineProps<{
  rawData: ReplicaSet | null
}>()

const emit = defineEmits<{
  (e: 'update:rawData', value: ReplicaSet): void
...
// Metadata state
const replicaSetLabels = ref<{ key: string; value: string }[]>([])
const replicaSetAnnotations = ref<{ key: string; value: string }[]>([])
...
const replicaSetNamespace = computed(() => {
  return props.rawData?.metadata?.namespace ?? ''
})

const containers = ref<ReplicaSetContainerFormState[]>([])
...
const syncFromRawData = (data: ReplicaSet | null) => {
  if (!data || isEmitting) return

  const normalized = normalizeReplicaSetSpec(data)
  ...
  // Metadata
  replicaSetLabels.value = kvObjectToArray(data.metadata?.labels)
  replicaSetAnnotations.value = kvObjectToArray(data.metadata?.annotations)
  ...
  dnsPolicy.value = podSpec?.dnsPolicy || KUBERNETES_DNS_POLICY.ClusterFirst
...
const handleFieldChange = () => {
  ...
  const rawObj = JSON.parse(JSON.stringify(toRaw(props.rawData))) as ReplicaSet
  ...
  // 1. General & Replicas
  spec.replicas = typeof replicas.value === 'number' ? replicas.value : 1
  spec.minReadySeconds = typeof minReadySeconds.value === 'number' ? minReadySeconds.value : 0

  // 2. Metadata
  rawObj.metadata.labels = kvArrayToObject(replicaSetLabels.value)
  rawObj.metadata.annotations = kvArrayToObject(replicaSetAnnotations.value)
        ...
        <TabPanel value="general" class="flex flex-col gap-10 max-w-7xl">
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div class="md:col-span-4 flex flex-col gap-1">
              ...
              <p class="text-xs text-muted-color leading-relaxed">
                Desired number of pod instances managed by this ReplicaSet.
              </p>
            </div>
            <div class="md:col-span-8 flex flex-col gap-4">
              <div class="flex flex-col gap-1.5">
                <label class="text-xs font-medium text-muted-color">Desired Replicas</label>
                <InputNumber
                  v-model="replicas"
                  showButtons
                ...
                <span class="text-[11px] text-muted-color">
                  Number of desired pods. Can be set to 0 to stop all pod replicas.
                </span>
              </div>
            </div>
          </div>

          <!-- Section 2: Timing & Availability -->
          <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div class="md:col-span-4 flex flex-col gap-1">
              <span class="text-xs font-semibold tracking-wider text-muted-color uppercase">
                Timing & Availability
              ...
              <p class="text-xs text-muted-color leading-relaxed">
                Minimum ready duration for pods to be considered available.
              </p>
            </div>
            <div class="md:col-span-8 flex flex-col gap-4">
              <div class="flex flex-col gap-1.5">
                ...
                />
                <span class="text-[11px] text-muted-color">
                  Minimum seconds for which a new pod should be ready without crashing before it is
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div class="md:col-span-4 flex flex-col gap-1">
              ...
              <p class="text-xs text-muted-color leading-relaxed">
                Pod selector labels. Immutable after ReplicaSet creation.
              </p>
            </div>
        ...
        <TabPanel value="metadata" class="flex flex-col gap-10 max-w-7xl">
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div class="md:col-span-4 flex flex-col gap-1">
              ...
              <p class="text-xs text-muted-color leading-relaxed">
                Labels and annotations attached to the ReplicaSet resource itself.
              </p>
            </div>
            <div class="md:col-span-8 flex flex-col gap-6">
              <div class="p-4 rounded-lg bg-(--bg-hover)/30 flex flex-col gap-3">
                <KeyValueEditor
                  v-model="replicaSetLabels"
                  title="ReplicaSet Labels"
                  add-label="Add Label"
                  @update:model-value="handleFieldChange"
                />
              </div>
              <div class="p-4 rounded-lg bg-(--bg-hover)/30 flex flex-col gap-3">
                <KeyValueEditor
                  v-model="replicaSetAnnotations"
                  title="ReplicaSet Annotations"
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div class="md:col-span-4 flex flex-col gap-1">
              ...
              <p class="text-xs text-muted-color leading-relaxed">
                Labels and annotations propagated to created ReplicaSet Pod instances.
              </p>
            </div>
            <div class="md:col-span-8 flex flex-col gap-6">
              <div class="p-4 rounded-lg bg-(--bg-hover)/30 flex flex-col gap-3">
        ...
        <TabPanel value="pod" class="flex flex-col gap-10 max-w-7xl">
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div class="md:col-span-4 flex flex-col gap-1">
              ...
              <p class="text-xs text-muted-color leading-relaxed">
                Host-level namespaces and DNS policies utilized by the Pod template.
              </p>
            </div>
            <div class="md:col-span-8 flex flex-col gap-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div class="md:col-span-4 flex flex-col gap-1">
              ...
              <p class="text-xs text-muted-color leading-relaxed">
                Node selector labels restricting which nodes this ReplicaSet runs on.
              </p>
            </div>
