...
<template>
  <div class="bg-(--bg-card) border border-(--border) rounded-xl p-6 shadow-sm flex flex-col gap-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex items-center gap-3 flex-wrap">
        ...
        <div class="relative min-w-64">
          ...
          <InputText
            v-model="searchQuery"
            placeholder="Search replicasets or images..."
    ...
    <DataTable
      ...
      currentPageReportTemplate="Showing {first} to {last} of {totalRecords} replicasets"
      @row-click="onRowClick"
    >
      <template #empty>
        <div class="text-center py-10 text-(--text-muted) flex flex-col items-center gap-2">
          <Info class="w-8 h-8 text-(--text-muted)/50" />
          <span>No replicasets found matching the filter criteria.</span>
        </div>
      </template>
