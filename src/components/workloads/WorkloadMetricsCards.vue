...
import { KUBERNETES_WORKLOAD_STATUS } from '@/constants/kubernetes'
...
const totals = computed(() => {
  ...
  deployments.value.forEach((d) => {
    ...
    if (d.status === KUBERNETES_WORKLOAD_STATUS.Running) running++
    else if (d.status === KUBERNETES_WORKLOAD_STATUS.Progressing) progressing++
    else if (d.status === KUBERNETES_WORKLOAD_STATUS.Failed) failed++
  })
